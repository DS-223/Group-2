...
def run_rfm_pipeline():
    ...
    rfm_base["last_transaction_date"] = pd.to_datetime(rfm_base["last_transaction_date"], errors="coerce")

    rfm_base["recency_days"] = (
            pd.Timestamp.now(tz='UTC').normalize() - rfm_base["last_transaction_date"]
    ).dt.days
